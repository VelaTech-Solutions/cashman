  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (!bankName || !BankDatesRules[bankName]) {
          setError("No valid bank rules found.");
          return;
        }

        // Extract dates using bank-specific BankDatesRules
        const cleanedData = (clientData.filteredData || []).map((line) => {
          const extractedDates = BankDatesRules[bankName](line) || [];
          return { original: line, extractedDates };
        });
    ...
    const handleExtractDates = async () => {
      ...
      const updatedTransactions = transactions.map((line) => {
        ...
        const segments = line.split(BankDatesRules); // Split transaction line into words/numbers
      ...
      try {
        ...
        const cleanedTransactions = transactions.map(line => {
          ...
          const segments = line.split(BankDatesRules);
          return segments
    ...
    const handleDeleteSegment = async (index, colIndex) => {
            ...
            const segments = updatedTransactions[index].split(BankDatesRules);
      ...
      <Table key={refreshKey}>
        <thead>
          ...
          <th className="px-2 py-1 w-[10px] border border-gray-600 text-left whitespace-nowrap">#</th> {/* Index Column */}
            {transactions.length > 0 &&
              Array.from({ length: transactions[0].split(BankDatesRules).length }).map((_, colIndex) => (
        ...
        <tbody>
          {transactions.map((transaction, index) => {
            const segments = typeof transaction === "string" ? transaction.split(BankDatesRules) : [];
            return (
