...
const Transportation = ({ transactions }) => {
  ...
  const transportationTransactions = transactions?.filter((txn) => txn.category?.toLowerCase() === "transportation") || [];

  // Group transactions by subcategory and month
  const groupedData = transportationTransactions.reduce((acc, txn) => {
