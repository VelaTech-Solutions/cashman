...
const Income = ({ transactions }) => {
  ...
  const incomeTransactions = transactions?.filter((txn) => txn.category?.toLowerCase() === "income") || [];

  // Group transactions by subcategory and month
  const groupedData = incomeTransactions.reduce((acc, txn) => {
    ...
    acc[subcategory][month] += txn.credit_amount || 0;
    acc[subcategory].total += txn.credit_amount || 0;
    acc[subcategory].count += txn.credit_amount ? 1 : 0;
    return acc;
  }, {});
