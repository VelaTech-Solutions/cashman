...
const Debt = ({ transactions }) => {
  ...
  const debtTransactions = transactions?.filter((txn) => txn.category?.toLowerCase() === "debt") || [];

  // Group transactions by subcategory and month
  const groupedData = debtTransactions.reduce((acc, txn) => {
    ...
    <div className="mb-8 border border-gray-600 rounded-lg overflow-hidden">
        ...
        <div className="relative max-h-[400px] overflow-y-auto" style={{ maxHeight: groupedData.length > 5 ? "400px" : "auto" }}>
            <table className="w-full border-collapse">
                ...
                <tr>
                    <th className="p-4 border border-gray-600 w-1/4 text-left">Subcategory</th>
                    {months.map((month) => (
                    <th key={month} className="p-4 border border-gray-600 w-1/12 text-center">{month}</th>
                    ))}
                    <th className="p-4 border border-gray-600 w-1/6 text-right">Total</th>
                </tr>
                </thead>
