...
export default function RestructureSettings() {
  const [restructureEnabled, setRestructureEnabled] = useState(false);
  const [restructurePlacements, setRestructurePlacements] = useState([]);
  ...
  const configDoc = () => doc(db, "settings", "budget", "config", "RestructureSettings");
  ...
  useEffect(() => {
    const fetchPlacements = async () => {
      ...
      if (snap.exists()) {
        ...
        const updates = {};
        if (!("restructureEnabled" in data)) updates.restructureEnabled = false;
        if (!("restructurePlacements" in data)) updates.restructurePlacements = [];
        ...
        setRestructureEnabled(data.restructureEnabled ?? false);
        setRestructurePlacements(data.restructurePlacements || []);
        setSheetPlacements(data.sheetPlacements || []);
      } else {
        const defaultData = {
          restructureEnabled: false,
          restructurePlacements: [],
          sheetPlacements: [],
        };
        await setDoc(ref, defaultData);
        setRestructureEnabled(false);
        setRestructurePlacements([]);
        setSheetPlacements([]);
      }
    };
    fetchPlacements();
  ...
  const processRowUpdate = (newRow) => {
    const updated = [...restructurePlacements];
    updated[newRow.id] = { cell: newRow.cell, sheet: newRow.sheet };
    setRestructurePlacements(updated);
    return newRow;
  };

    const rows = restructurePlacements.map((item, index) => ({
    id: index,
    ...
    const columns = [
        ...
        { field: "sheet", headerName: "Sheet Name", flex: 1 },
        {
            ...
            <GridActionsCellItem
                ...
                label="Delete"
                onClick={async () => {
                const updated = restructurePlacements.filter((_, i) => i !== params.id);
                setRestructurePlacements(updated);
                try {
                    await updateDoc(configDoc(), {
                    restructurePlacements: updated,
                    });
                } catch (err) {
  ...
  return (
    <Box sx={{ width: '100%', maxWidth: '1700px', mx: 'auto' }}>
      <Stack spacing={2}>
        <Typography variant="h6">Restructure Insurance Cell Placements Settings</Typography>
