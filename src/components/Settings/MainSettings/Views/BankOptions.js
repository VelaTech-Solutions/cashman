...
export default function BankOptions() {
  ...
  const configDoc = (bank) => doc(db, "settings", "bankOptions", bank, "config");
  

  const fieldsConfig = [
    { key: 'amountRegex', label: 'Amount Regex', type: 'string' },
    { key: 'dateRegex', label: 'Date Regex', type: 'string' },
    { key: 'descriptionRegex', label: 'Description Regex', type: 'string' },
                      ...
                      {fieldsConfig.map(({ key, label }) => (
                        <Box key={key}>
                          <Typography variant="body2" color="text.secondary">
                            <strong>{label}:</strong> {entryConfig?.[key] || <em>—</em>}
                          </Typography>
                        </Box>
                      ))}
                    </Stack>
