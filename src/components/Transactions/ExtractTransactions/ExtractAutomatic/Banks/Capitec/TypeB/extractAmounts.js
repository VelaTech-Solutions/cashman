...
const extractAmounts = async (clientId, bankName, type) => {
  ...
  try {
    ...
    filteredData.forEach((line, index) => {
      ...
      let extracted = extractAmountsFromText(line);

      if (extracted.length === 1) {
        // Only one amount found, ignore for now
        return;
      }

      if (extracted.length === 2) {
        // Two values found, assume format: [credit/debit, balance]
        // Inject 0.00 in the middle as fees
        extracted.splice(1, 0, "0.00");
      }
      ...
      // Destructure amounts with fallback to "0.00"
      const credit_debit_amount = extracted[0] || "0.00";
      const fees_amount = extracted[1] || "0.00";
      const balance_amount = extracted[2] || "0.00";

      // Strip whitespace from amounts
      const credit_debit_amountStripped = credit_debit_amount.replace(/[\s]/g, "");
      const fees_amountStripped = fees_amount.replace(/[\s]/g, "");
      const balance_amountStripped = balance_amount.replace(/[\s]/g, "");

      // Remove the matched amounts from the line to leave a clean description
