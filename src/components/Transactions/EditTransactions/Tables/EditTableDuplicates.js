// src/components/Transactions/EditTransactions/Tables/EditTableDuplicates.js
import React from "react";

// Utils imports
import BaseTable from "../Utils/BaseTable";

const EditTableDuplicates  = ({ transactions, onEditClick }) => {
    ...
    // if date1, date2, description, credit_amount, debit_amount, balance_amount are all the same, then it's a duplicate
    const filtered = transactions.filter(
        (tx) =>
          new Set(transactions.map(tx => tx.date1)).size === 1 &&
          new Set(transactions.map(tx => tx.date2)).size === 1 &&
          new Set(transactions.map(tx => tx.description)).size === 1 &&
          new Set(transactions.map(tx => tx.credit_amount)).size === 1 &&
          new Set(transactions.map(tx => tx.debit_amount)).size === 1 &&
          new Set(transactions.map(tx => tx.balance_amount)).size === 1
      );
    const headers = (
        <tr>
    ...
    return (
        <div>
            ...
            <p className="mt-4 text-gray-400">
                Total Duplicates: {filtered.length}
            </p>
        </div>
        );
    };
export default EditTableDuplicates