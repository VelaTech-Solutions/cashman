...
const EditTableMissingDates = ({ clientId }) => {
  ...
  const handleRemoveMissingDates = async () => {
    const confirm = window.confirm("Remove all transactions with missing Date1 and Date2?");
    if (!confirm) return;
    ...
    alert("Removed and archived all transactions with missing dates.");
  };
  ...
  return (
    <div>
      <h1 className="text-sm font-semibold mb-4">Transactions with Missing Dates</h1>
      <button
        className="bg-red-600 hover:bg-red-700 text-white py-1 px-3 rounded mb-4"
        onClick={handleRemoveMissingDates}
      >
        Remove All
      </button>
      <BaseTable headers={headers} rows={rows} />
      <p className="mt-4 text-gray-400">
        Missing Dates: {filteredTransactions.length}
      </p>
    </div>
  );
};

export default EditTableMissingDates;
