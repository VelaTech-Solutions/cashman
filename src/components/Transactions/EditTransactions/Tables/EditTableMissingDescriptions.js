// src/components/Transactions/EditTransactions/Tables/EditTableMissingDescriptions.js
import React from "react";

// Utils imports
import BaseTable from "../Utils/BaseTable";

const EditTableMissingDescriptions  = ({ transactions, onEditClick }) => {
    if (!transactions || transactions.length === 0) {
      return <p className="text-gray-400">No transactions found.</p>;
    }
    const filtered = transactions.filter(
        (tx) => !tx.description || tx.description.trim() === ""
      );
    const headers = (
        <tr>
    ...
    return (
        <div>
            ...
            Missing Descriptions: {filtered.length}
            </p>
        </div>
        );
    };
